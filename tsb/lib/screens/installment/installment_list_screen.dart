...
import 'installment_add_screen.dart';
...
class _InstallmentListScreenState extends State<InstallmentListScreen> {
    ...
    try {
      ...
      if (results.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('কোন কিস্তি পাওয়া যায়নি')),
        );
      }
    } catch (e) {
      setState(() {
  ...
  Future<void> _deleteInstallment(int id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('আপনি কি এই কিস্তিটি ডিলিট করতে চান?'),
        actions: [
          TextButton(
    ...
    if (confirm == true) {
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('কিস্তি সফলভাবে ডিলিট করা হয়েছে')),
        );
        _loadInstallments();
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('কিস্তি তালিকা'),
        actions: [
          IconButton(
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'সদস্য, পণ্য বা ফোন নম্বর দিয়ে খুঁজুন...',
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('কিস্তি যোগ করুন'),
      ),
    );
  }
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    _getBanglaStatus(installment.status),
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 12,
                      fontWeight: FontWeight.bold,
            ...
            Row(
              children: [
                Expanded(
                  child: _buildInfoColumn(
                    'মাসিক কিস্তি',
                    '৳${installment.monthlyInstallmentAmount?.toStringAsFixed(2) ?? 'নেই'}',
                  ),
                ),
                Expanded(
                  child: _buildInfoColumn(
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: _buildInfoColumn(
                    'এজেন্ট',
                    installment.givenProductAgent?.name ?? 'নেই',
                  ),
                ),
              ],
            ),
